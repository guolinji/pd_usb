
      
	initial begin
      
                wait( initial_done == 1);
		pe2pl_hard_reset_ack_m = 1;
	        @(posedge clk);
		pe2pl_tx_en_m = 1;
		pe2pl_tx_sop_type_m = 3'h3;
	        @(posedge clk);
		pe2pl_tx_en_m = 0;
		$display($stime, "-->Master Send hardreset message begin.");

                wait( pl2pe_tx_ack_m == 1);
                $display($stime, "-->Master Send hardreset message done, the resutl is %h.", pl2pe_tx_result_m);
                //$display($stime, "-->Send message done");

	end
      
      
	initial begin
      
                wait( pl2pe_rx_en_s == 1);
                $display($stime, "-->Slave receive message , the resutl is %h, the sop type is %h, the message type is %b", pl2pe_rx_result_s, pl2pe_rx_sop_type_s, pl2pe_rx_type_s);

	end
      
      
      

